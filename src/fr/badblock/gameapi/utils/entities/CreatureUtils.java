...
public class CreatureUtils {
	 ...
	 * 
	 * @param location La position du spawn
	 * @param clazz Le type d'entité à faire spawn
	 */
	 ...
	 * @param radius La distance maximum entre l'entité et la position
	 * 
	 ...
	 * @param from La location à changer
	 * @param to La location à regarder
	 * @param aiming La précision (100 = précision totale)
	...
	public static Location lookAt(Location from, Location to, float aiming){
		...
		if(yawAiming > 0) yawAiming = MathsUtils.integerRandomInclusive((int) yawAiming, (int) -yawAiming);
		
		float pitchAiming = 180 - (aiming * 180 / 100);
		if(pitchAiming > 0) pitchAiming = MathsUtils.integerRandomInclusive((int) pitchAiming, (int) -pitchAiming);
	 ...
	 * @param entityId L'UUID de l'entité.
	...
	/**
	 * Recherche la version modifiée (par l'API) d'une créature. Si l'API n'a pas sa classe de register ou que l'entité n'est pas gérée, retourne null.

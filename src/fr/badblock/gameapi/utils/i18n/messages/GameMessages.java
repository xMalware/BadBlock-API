...
/**
 * Liste de messages utilisés par l'API ou à déstination des plugins annexes pour unifier le système de traduction (pas de doublon) et
...
public class GameMessages {
	/**
	 * Lorsqu'un spectateur s'éloigne, le message qui lui est affiché. Utile uniquement pour l'API.
	 * @return Le message
	 */
	...
	public static TranslatableString deathEventMessage(NormalDeathEvent e){
		return new TranslatableString("deathmessages.normal." + e.getLastDamageCause().name().toLowerCase(), e.getPlayer().getName());
	}
	
	public static TranslatableString deathEventMessage(FightingDeathEvent e){
		if(e.getKiller().getType() == EntityType.PLAYER){
			return new TranslatableString("deathmessages.pvp." + e.getLastDamageCause().name().toLowerCase(), e.getPlayer().getName(), e.getKiller().getName());
		} else {
			return new TranslatableString("deathmessages.pve." + e.getLastDamageCause().name().toLowerCase(), 
					new TranslatableWord("entities." + e.getKiller().getType().name().toLowerCase(), false, WordDeterminant.UNDEFINED));
		}
	}
